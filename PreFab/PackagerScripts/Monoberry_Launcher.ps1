...
$temp='tmp\';
$locale='tmp\locale';
$exe='{ndk host folder path}\win32\x86\usr\bin\blackberry-nativepackager.bat';
$address="{device IP}";
$password="{device password}"
$scriptPath = $MyInvocation.mycommand.path;
$barName="{app bar file}.bar";
...
function InstallPackage()
{
	...
	$arguments= [System.String]::Format('-installApp -device {0} -password {1} "{2}"', $address, $password, $barPath);
...
function LaunchApplication()
{
	...
	$arguments= [System.String]::Format('-launchApp -device {0} -password {1} "{2}"', $address, $password, $barPath);
	RunProcess $exe $arguments;
}
